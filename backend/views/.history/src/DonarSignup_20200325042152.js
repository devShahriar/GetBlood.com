...
export default class DonarSignup extends React.Component {
  ...
  render() {
    ...
    if(this.state.validEmail){
      label= <Label pointing='left'>valid</Label>
    }
    if(!this.state.validEmail && this.state.validEmail!==0){
      label=<Label pointing='left'>invalid email</Label>
    }
    if(this.state.validEmail.length===0)
    {
      label =<Label pointing='left'> * Required field</Label>
    }
    return (
