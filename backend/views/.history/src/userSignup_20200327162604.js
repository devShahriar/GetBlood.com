...
export default class DonarSignup extends React.Component {
  ...
  passValid=()=>{
    if(/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{4,8}$/.test(this.state.password)){
      return true;
    }
  }
  render() {
    ...
    if(!passValid){
       passlabel=<Label color='red' pointing='left'> Password must be at least 4 characters, 
      ...
      if(this.state.password!==this.state.confirmpassword){
        passlabel =<Label color='red' pointing='left' >password field and comfirm password field doesnt match</Label>
      }
    }
